...
export const REST_API_LOG = {
	logIn: async (data: { email: string; password: string }) => {
		const result = await axios.post(`${IP_ADDRESS}:${PORT}/api/members/login`, data, { headers: {"Content-Type": "application/json", withCredentials: true }});
		return result.data;
	},
...
export const REST_STOCK = {
	order : async (token: string, memberIdx : number, stockCode: number, data: {stockOrderPrice: number, stockOrderCount: number}) => {
		const result = await axios.post(`${IP_ADDRESS}:${PORT}/api/orders/${memberIdx}/${stockCode}/new`, data, { headers: {"Authorization" : `Bearer ${token}`}});
		return result;
	},
	myOrders : async (token: string, memberIdx: number) => {
		const result = await axios.get(`${IP_ADDRESS}:${PORT}/api/members/${memberIdx}/orders`, { headers: {"Authorization" : `Bearer ${token}`}});
		console.log(result);
		return result;
	},
	all : async (token: string) => {
		const result = await axios.get(`${IP_ADDRESS}:${PORT}/api/stocks/`, { headers: {"Authorization" : `Bearer ${token}`, withCredentials: true}});
		return result;
	}
}
...
export const ADMIN = {
	newStock : async (data : { stockName : string, price: number}, token: string) => {
		const result = await axios.post(`${IP_ADDRESS}:${PORT}/api/stocks/new`, data, { headers: {"Authorization" : `Bearer ${token}`}});
		return result;
	}
}
