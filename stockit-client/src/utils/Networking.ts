...
export const REST_API_LOG = {
	logIn: async (data: { email: string; password: string }) => {
		const result = await axios.post(`${IP_ADDRESS}:${PORT}/api/members/login`, data, { headers: {"Content-Type": "application/json"}});
		return result.data;
	},
...
export const REST_STOCK = {
	...
	all : async (token: string) => {
		const result = await axios.get(`${IP_ADDRESS}:${PORT}/api/stocks/`, { headers: {"Authorization" : `Bearer ${token}`}});
		return result;
	}
}
